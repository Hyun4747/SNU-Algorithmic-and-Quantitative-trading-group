...
class SimpleVB(EventDrivenStrategy):
    def __init__(
        self,
        symbol: Symbol,
    ...
    @property
    def slug(self) -> str:
        return f"simplevb{self.symbol.base_currency}"
    ...
    def update_indicators(self) -> None:
        ...
        self.range = self.make_indicator(self.calculate_range(n_candles=N_CANDLES_PER_DAY), plot=False)
...
presets: list[StrategyPreset] = [
    StrategyPreset(
        name="SimpleVB (BTC)",
        description="Volatility Breakout without timeframe diversification",
        strategies=[SimpleVB(symbol=Symbol.BTC)],
    ),
    StrategyPreset(
        name="SimpleVB (BTC and ETH)",
        description="Volatility Breakout without timeframe diversification",
        strategies=[SimpleVB(symbol=symbol) for symbol in [Symbol.BTC, Symbol.ETH]],
    ),
    StrategyPreset(
        name="SimpleVB (Top10)",
        description="Volatility Breakout without timeframe diversification",
        strategies=[SimpleVB(symbol=symbol) for symbol in Symbol.top10()],
    ),
    StrategyPreset(
        name="SimpleVB (All)",
        description="Volatility Breakout without timeframe diversification",
        strategies=[SimpleVB(symbol=symbol) for symbol in Symbol.all()],
    ),
]
...
if __name__ == "__main__":
    ...
    debug_preset = StrategyPreset(
        name="SimpleVB (Debug)",
        strategies=[SimpleVB(symbol=symbol) for symbol in [Symbol.BTC]],
    )
